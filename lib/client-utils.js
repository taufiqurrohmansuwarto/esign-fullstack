...
export const flattenDeep = (data) => {
  return data.reduce((acc, val) => Array.isArray(val) ? acc.concat(flattenDeep(val)) : acc.concat(val), []);
}

export const roleType = (data, userId) => {
  const currentRole = data?.Recipient?.find(recipient => recipient?.signatory_status === 'PENDING' && recipient?.recipient_id === userId);
...
export const recipientPosition = (data, userId) => {
  const listRecipient = data?.Recipient?.filter(recipient => recipient?.signatory_status === 'PENDING');
  const myCurrentPosition = listRecipient?.find(recipient => recipient?.recipient_id === userId)?.sequence;
...
// digunakan untuk request from othres
