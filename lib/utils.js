...
const createStamp = async (data) => {
  ...
  const pengantar = "Ditandatangani secara elektronik oleh";
...
// create remove document usign minio

const removeDocument = async ({ minio, filename }) => {
  try {
    const result = await minio.removeObject("storage", `/esign/${filename}`);
    return result;
  } catch (error) {
    console.log(error);
  }
